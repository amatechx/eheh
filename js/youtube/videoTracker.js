const videoTracker = (function(){
    ...
    let tasks = {
        ...
        enableNotifications: {
            required: false,
            isCompleted: false,
            text: getLocaleMsg('ytTask_enableNotifications')
        },
        postComment: {
            required: false,
            isCompleted: false,
    ...
    let getStatusUIHtml = () => {
        ...
        for(let taskIndex in tasks) {
            ...
            if(!task.required || taskIndex == 'enableNotifications') {
                continue;
            }

            let taskText = task.text;
            if (taskIndex === 'likeComment') {
                task.commentType === 'pinned' && (taskText = 'Like pinned comment');
                task.commentType === 'reply-to-pinned' && (taskText += ' (look for it in replies under pinned comment)');
                task.commentType === 'reply-to-regular' && (taskText += ' (look for it in replies under regular comments)');
            }
            let extraHtml = '';
            if(taskIndex == 'watchVideo') {
                ...
                `;
            } else if (taskIndex === 'likeComment' && task.commentType != 'pinned') {
                extraHtml += `
                <div class="eh-task-info">
                    <div class="commentInfo">
    ...
    let markTaskComplete = async (taskId) => {
        ...
        $('.ehTask-' + taskId).find('.eh-task-status-icon i').addClass('fa-globe').removeClass('fa-hand-peace-o');
        ...
        let autoWatchingTaskCompleteHandler = async () => {
            ...
            if (response && response.userCanContinueAfter === 0 && response.ipCanContinueAfter === 0){
                if (settings.checkNotifications == 1 && !isMobile()) {
                    const videoIds = await getVideoIdsFromNotifications();
                    await pushVideosToQueue('notifications', videoIds);
                }
        ...
        if (isAllCompleted) {
            chrome.runtime.sendMessage({
                ...
                adsInfo: {
                    ...
                    adsMet: adsInfo.advertisedProducts.size,
                },
            }, async response => {
                if (response.isSuccessful) {
                    if(isTabAutoWatching){
                        clear();
                        await autoWatchingTaskCompleteHandler();
                        return;
                    }
                    else{
    ...
    let videoTimeUpdateListener = async () => {
        ...
        if($('video:first').parents('.html5-video-player').hasClass('ad-showing')) {
            if(isTabAutoWatching){
                ...
                const countDown = $('.ytp-ad-preview-container.countdown-next-to-thumbnail:visible');
                const productName = $('.video-ads .ytp-ad-visit-advertiser-button').attr('aria-label');

                if (adsInfo.adsWatched < needToWatchAdsCount && video.duration <= 30) {
                    if (skipBtn.length) {
                        skipBtn.remove();
                    }
                    if (!adsInfo.advertisedProducts.has(productName)) {
                        adsInfo.adsWatched += 1;
                    }
                } else if (skipBtn.length && !countDown.length) {
                    skipBtn.simulate("click");
                }
                adsInfo.advertisedProducts.add(productName);
            }
            else{
                showSkipAdHighlight();
            }
        ...
        if (!videoDurationChecked && video.duration < tasks.watchVideo.watchDuration) {
            videoDurationChecked = true;
            log('video is too short');
            await skipTask(true);
            window.location = "https://www.youtube.com";
        }
    ...
    let findAndClickTargetLink = async (videoId) => {
        let targetLink = null;
        const targetLinks = [];
        const drawerPlaylistLink = $('.iv-drawer a[href*="list="]');
        const drawerDirectLink = $(`.iv-drawer a[href*="${videoId}"]`);
        const endVideoPlaylistLink = $(`.ytp-ce-element.ytp-ce-video a[href*="list="]`);
        const endVideoDirectLink = $(`a.ytp-ce-covering-overlay[href*="${videoId}"]`);
        const suggestionsDirectLink = $(`#related a[href*="${videoId}"]`);
        const suggestionsPlaylistLink = $(`#related a[href*="list="]#thumbnail`);

        (endVideoPlaylistLink.length > 0) && (targetLinks.push(endVideoPlaylistLink.first()));
        (drawerPlaylistLink.length > 0) && (targetLinks.push(drawerPlaylistLink.first()));
        targetLinks.sort(() => Math.random() - 0.5);
        (suggestionsDirectLink.length > 0) && (targetLinks.push(suggestionsDirectLink.first()));
        (drawerDirectLink.length > 0) && (targetLinks.push(drawerDirectLink.first()));
        (endVideoDirectLink.length > 0) && (targetLinks.push(endVideoDirectLink.first()));
        (suggestionsPlaylistLink.length > 0) && (targetLinks.push(suggestionsPlaylistLink.first()));
        if (targetLinks.length) {
            targetLink = targetLinks.pop();
            await sleep(2000);
            targetLink.simulate("click");
            chrome.runtime.sendMessage({
    ...
    let startTracking = () => {
        ...
        pingInterval = setInterval(() => {
            chrome.runtime.sendMessage({action: 'sendTaskInProgressPing', taskId: currentTask.taskId}, (r) => {
                console.log(r);
            });
        ...
        if(
          ...
          tasks['enableNotifications'].required ||
          ...
          tasks['likeComment'].required
            ...
            lsInterval = setInterval(() => {
                ...
                tasks['enableNotifications'].required && checkNotificationStatus();
    ...
    let checkLikeToCommentStatus = async () => {
        if (isTabAutoWatching) {
            if(!likeCommentScheduled){
                ...
                setTimeout(() => {
                    new Promise((resolve) => {
                      ...
                      .then(async () => {
                        if (tasks['likeComment'].commentType == 'reply-to-regular' || tasks['likeComment'].commentType == 'regular') {
                            await new Promise((resolve) => {
                                $("html, body").animate({scrollTop: $("ytd-comments#comments").position().top + 1500}, 2000, () => {
                                    resolve();
                                });
                            });
                            await sleep(2000);
                        ...
                        let tmpBtn = {};
                        let error = '';
                        // seeking for the comment depending on its type
                        if (tasks['likeComment'].commentType == 'reply-to-pinned') {
                            const pinnedComment = $(`#comments #comment`).first();
                            if (pinnedComment.length > 0 && pinnedComment.find('#pinned-comment-badge').length > 0) {
                                ...
                                tmpBtn = $(`#comments #body:contains("${tasks['likeComment'].comment}") #like-button`);
                            } else error = 'pinned comment not found';
                        }
                        else if (tasks['likeComment'].commentType == 'pinned') {
                            const pinnedComment = $(`#comments #comment`).first();
                            if (pinnedComment.length > 0 && pinnedComment.find('#pinned-comment-badge').length > 0) {
                                tmpBtn = $(`#comments #body #like-button`);
                            } else error = 'pinned comment not found';
                        }
                        else if (tasks['likeComment'].commentType == 'reply-to-regular') {
                            $(`ytd-comments#comments #contents ytd-comment-thread-renderer #more-replies`).simulate('click');
                            // wait until all replies are loaded
                            await new Promise(resolve => {
                                const check = setInterval(() => {
                                    if ($('#comments #replies #spinnerContainer.active').length === 0) {
                                        clearInterval(check);
                                        resolve()
                                    }
                                }, 1000);
                        ...
                        if (tmpBtn.length) {
                            likeCommentBtn = tmpBtn.first();
                            await new Promise(resolve => {
                                $("html, body").animate({ scrollTop: likeCommentBtn.position().top - 200 }, 1000, () => resolve());
                            })
                            await sleep(1000);
                            ...
                            console.log('likeCommentBtnNotFound ', error);
                            await skipTask(true);
                            window.location = "https://www.youtube.com";
                        }
                    })
            ...
            // seeking for the comment depending on its type
            if (tasks['likeComment'].commentType == 'reply-to-pinned') {
                const pinnedComment = $(`ytd-comment-thread-renderer`).first();
                if (pinnedComment.length > 0 && pinnedComment.find('#pinned-comment-badge').length > 0) {
                    tmpBtn = pinnedComment.find(`#loaded-replies #body:contains("${tasks['likeComment'].comment}") #like-button`);
                }
            }
            else if (tasks['likeComment'].commentType == 'pinned') {
                const pinnedComment = $(`#comments #comment`).first();
                ...
                tmpBtn = $(`#comments #replies #body:contains("${tasks['likeComment'].comment}") #like-button`);
            }
            else { // regular comment
                tmpBtn = $(`#comments #body:contains("${tasks['likeComment'].comment}") #like-button`);
            }
            if (tmpBtn.length) {
                ...
                if($('#likeCommentHighlight').length === 0 && !likeCommentBtn.hasClass('style-default-active')) {
                    likeCommentBtn.after(likeCommentStatusHTML);
                }
            }
        }
        ...
        if (status) {
            ...
            !isTabAutoWatching && $('#likeCommentHighlight').length === 0 && likeCommentBtn.after(likeCommentStatusHTML);
        }
    ...
    let checkReplyToPinnedStatus = async () => {
        ...
        if (isTabAutoWatching) {
            if (tasks['likeComment'].required && !tasks['likeComment'].isCompleted) {
                return;
            }
            if(!replyScheduled){
                replyScheduled = true;
                replyTimeout = Math.max(commentTimeout + 10, 10);
                setTimeout(() => {
                    new Promise((resolve) => {
                        $("html, body").animate({ scrollTop: $("ytd-comments#comments").position().top - 200 }, 1000, () => {
                            resolve();
                        ...
                        if (tmpPinnedComment.length > 0 && tmpPinnedComment.find('#pinned-comment-badge').length > 0) {
    ...
    let checkSubscribeStatus = async () => {
        let btn = $('#primary.ytd-watch-flexy #top-row #subscribe-button tp-yt-paper-button');
        if (!btn) {
            return;
        }
                ...
                if(!subscriptionScheduled) {
                    ...
                    setTimeout(() => {
                        if (!tasks['subscribeChannel'].isCompleted) {
                            $('#primary.ytd-watch-flexy #top-row #subscribe-button tp-yt-paper-button:not([subscribed])').simulate("click");
                        }
        ...
        if(typeof status != 'undefined') {
            ...
            tasks['enableNotifications'].required && markTaskIncomplete('enableNotifications');
        }
    };

    let checkNotificationStatus = async () => {
        if (!tasks['subscribeChannel'].isCompleted || tasks['enableNotifications'].isCompleted) {
            return;
        }
        if (isMobile()) {
            markTaskComplete('enableNotifications');
            return;
        }
        $('ytd-subscribe-button-renderer ytd-subscription-notification-toggle-button-renderer button#button').simulate('click');
        await sleep(500);
        const enableNotificationsBtn = document.querySelector('ytd-menu-service-item-renderer.style-scope.ytd-menu-popup-renderer');
        if (!enableNotificationsBtn) {
            return;
        }
        enableNotificationsBtn.classList.add('need-to-click');
        $('ytd-menu-popup-renderer ytd-menu-service-item-renderer.need-to-click').simulate('click');
        enableNotificationsBtn && enableNotificationsBtn.classList.remove('need-to-click');
        markTaskComplete('enableNotifications');
    };
    ...
    let checkCommentStatus = () => {
        ...
        if(isTabAutoWatching){
            if (
              tasks['likeComment'].required && !tasks['likeComment'].isCompleted ||
              tasks['replyToPinned'].required && !tasks['replyToPinned'].isCompleted
            ) {
                return;
            }
            if(!commentScheduled) {
    ...
    let showSpinnerInComments = async () => {
        const commentBlock = await waitForElement('ytd-item-section-renderer#sections');
        commentBlock.classList.add('hidden-comments');
        const spinner = await waitForElement('ytd-comments#comments paper-spinner-lite');
        spinner.style.display = 'block';
        spinner.style.marginTop = '40px';
        spinner.removeAttribute('aria-hidden');
        spinner.classList.add('ce-spinner');
        const spinnerContainer = await waitForElement('.ce-spinner #spinnerContainer');
        spinnerContainer.classList.add('active');
    };

    let hideSpinnerInComments = async () => {
        const spinner = await waitForElement('ytd-comments#comments paper-spinner-lite');
    ...
    let init = async task => {
        ...
        if (task.visitType == 'suggested-videos' && getParameterByName('list') && videoId !== task.videoId) {
            await sleep(4000);
            const target = $(`ytd-playlist-panel-renderer#playlist a[href*=${task.videoId}]`);
            if (target.length > 0) {
                await sleep(1000);
                target.simulate('click');
            } else {
                await sleep(2000);
                window.location.replace('https://www.youtube.com/watch?v=' + task.videoId);
            }
        }
        ...
        if (task.visitType == 'competitor-videos' && videoId == getParameterByName('v', task.currentCompetitorVideoURL || '')) {
            await sleep(1000);
            let targetLink = $(`#related a[href*="${task.videoId}"]`);
            if (targetLink.length === 0) {
                window.location.href = "https://www.youtube.com/";
                return;
            } else {
                targetLink.first().simulate('click');
            }
        }

        if (task.visitType == 'suggested-videos' && videoId == getParameterByName('v', task.baitVideoInfo.videoURL || '') && !getParameterByName('list') ) {
            await sleep(2000);
            await findAndClickTargetLink(task.videoId);
            return;
        }
    ...
    let clear = () => {
        ...
        waitForElement('ytd-item-section-renderer#sections')
          .then((el) => {
              el.classList.remove('hidden-comments');
          });
        hideSpinnerInComments();
